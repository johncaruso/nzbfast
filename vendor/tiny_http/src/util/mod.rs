...
pub use self::deadline::{DeadlineReader, DeadlineWriter, DesyncGuard};
...
// `SequentialWriter` is only named by the un-vendored TLS paths; keep the
// re-export for API fidelity with upstream.
