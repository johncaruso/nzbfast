...
mod tests {
    ...
    /// The original bit-at-a-time peek, kept as the differential reference
    /// for the word-at-a-time `BitReader::get_bits`.
    fn get_bits_reference(
        input: &[u8],
        bit_pos: usize,
        final_input: bool,
    ) -> super::Result<u32> {
        ...
        for i in 0..16 {
            ...
            let byte = match input.get(bit_index / 8).copied() {
                Some(byte) => byte,
                None if final_input => 0,
                None => return Err(super::Error::NeedMoreInput),
            };
    ...
    /// Every alignment, every buffer-end position, both `final_input` states:
    /// value and `NeedMoreInput` must match the bit-at-a-time reader exactly.
    #[test]
    fn word_at_a_time_get_bits_matches_bitwise_reference() {
        ...
        for len in 0..=pattern.len() {
            ...
            for &final_input in &[false, true] {
                let mut reader = super::BitReader::new(input);
                reader.final_input = final_input;
                // Walk past the end so the truncated-window paths are covered.
                for bit_pos in 0..(len * 8 + 24) {
                    reader.bit_pos = bit_pos;
                    assert_eq!(
                        reader.get_bits(),
                        get_bits_reference(input, bit_pos, final_input),
                        "len {len}, bit_pos {bit_pos}, final_input {final_input}"
                    );
                }
            }
        }
...
impl BitReader {
    ...
    /// Peek the next 16 bits, MSB first.
    ///
    /// The window always lies inside three consecutive bytes, so it is one
    /// shift out of a 24-bit word rather than sixteen bounds-checked loads.
    fn get_bits(&self) -> Result<u32> {
        let index = self.bit_pos / 8;
        let shift = self.bit_pos % 8;
        let tail = self.input.get(index..).unwrap_or(&[]);
        if let [first, second, third, ..] = *tail {
            let window =
                u32::from(first) << 16 | u32::from(second) << 8 | u32::from(third);
            return Ok((window >> (8 - shift)) & 0xffff);
        }
        // Buffer end: a byte-aligned window never reaches the third byte, and
        // past `finish()` anything beyond the input reads as zero.
        let mut window = 0u32;
        for offset in 0..3 {
            let byte = match tail.get(offset).copied() {
                Some(byte) => byte,
                None if shift == 0 && offset == 2 => 0,
                None if self.final_input => 0,
                None => return Err(Error::NeedMoreInput),
            };
            window = (window << 8) | u32::from(byte);
        }
        Ok((window >> (8 - shift)) & 0xffff)
    }
