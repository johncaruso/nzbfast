...
| `NZBFAST_DIAL_RACE` | `1` races TCP connects to the top two resolved server addresses (50 ms stagger, first connect wins) to dodge bad pool nodes | off | Experimental |
...
| `NZBFAST_CRC_RETRY` | Validates each article body in the pool (yEnc CRC plus declared part number vs the requested segment) and refetches a bad body from a DIFFERENT server once before accepting the damage; off, a corrupt article rides to PAR2 repair. Costs one extra decode per article while on, so the default follows whether an elsewhere exists: on with 2+ enabled servers, off single-server. `1` forces on, `0` forces off | on with 2+ servers | Supported |
