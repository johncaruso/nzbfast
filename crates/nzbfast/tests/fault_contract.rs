...
/// `crates/nzbkit/tests/tls_chaos.rs`; what this leg adds is the five
...
fn every_profile_is_pinned_or_excused() {
    ...
    assert!(
        ...
        "chaos-serve profiles with no place in the fault contract: {unclassified:?}.\n\
         Add a leg to tests/fault_contract.rs (a TLS profile from §129 3b needs one - \
         see the acceptance criteria in research/SPEC-2026-08-08-129-phase3.md), or add \
         it to NOT_IN_PROCESS with the reason it belongs to the bench matrix only."
    );
