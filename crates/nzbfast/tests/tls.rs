...
/// Readiness = OUR https banner in OUR log (a bare connect can catch a
/// stranger on a recycled port - see index_size_cap.rs). Also the
/// banner-says-https assertion.
fn wait_ready(dir: &Path, child: &mut KillOnDrop, port: u16) {
    let banner = format!("open the dashboard at  https://localhost:{port}/");
    for _ in 0..300 {
        if std::fs::read_to_string(dir.join("daemon.log"))
            .unwrap_or_default()
            .contains(&banner)
    ...
    panic!(
        "daemon never printed the https banner: {}",
        std::fs::read_to_string(dir.join("daemon.log")).unwrap_or_default()
    );
}
...
fn dashboard_and_api_answer_over_https() {
    ...
    let port = free_port();
    let mut daemon = spawn_daemon(&dir, port, &cert, &key);
    wait_ready(&dir, &mut daemon, port);
