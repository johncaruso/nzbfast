...
/// OS-assigned free port for a daemon under test (see queue_soak.rs for
/// why pid-derived schemes are unsafe).
fn free_port() -> u16 {
...
/// (banner-in-own-log readiness; see queue_soak.rs for the port race).
    ...
    tokio::task::spawn_blocking(move || {
        ...
        for _ in 0..300 {
            ...
            if let Some(slots) = q["slots"].as_array() {
                ...
                assert!(
                    downloading <= 1,
                    "two Downloading rows in one snapshot: {slots:?}"
                );
