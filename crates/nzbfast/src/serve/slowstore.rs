...
impl Tune {
    ...
    pub(crate) fn from_settings(v: &Value) -> Tune {
...
impl Judge {
    pub(crate) fn new(tune: Tune) -> Judge {
        Judge {
            tune,
            hist: VecDeque::new(),
            floor_ms: 0,
    ...
    pub(crate) fn rearm(&mut self, at_ms: u64) {
        self.floor_ms = at_ms;
        self.latched = false;
    }
    ...
    pub(crate) fn observe(&mut self, s: Sample) -> Option<Evidence> {
        if s.span_ms == 0 {
            return None;
        }
        self.hist
...
pub(super) fn payload(d: &Daemon) -> Value {
...
pub(super) fn release(d: &Arc<Daemon>, why: &str) {
...
/// releases the queue.
pub(super) fn heal(d: &Arc<Daemon>, probe: &Probe) -> bool {
...
pub(crate) fn step(d: &Daemon) -> Step {
