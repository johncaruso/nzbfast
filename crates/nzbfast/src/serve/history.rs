...
pub(super) fn history_change_cat(d: &Daemon, id: &str, cat: &str) -> Value {
    ...
    // physically relocated the payload and then could not write the
    // queue file restores the OLD record at restart, pointing every
    // later delete/retry/import at the emptied source while the bytes
    // sit unclaimed at the destination (Codex sweep 5 Aug M5). The
    // live record is right either way - what failed is durability, and
    // the caller has to hear it with both paths in hand.
    let durability = (!d.save_queue()).then(|| match &moved {
        Some(dest) => format!(
            "the updated record could not be written to the queue file - after \
        ...
        None => format!(
            "the new category could not be written to the queue file - it \
