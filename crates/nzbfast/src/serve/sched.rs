...
#[derive(Debug, Clone)]
pub(super) struct SchedEntry {
    /// Mon=0 .. Sun=6.
    days: [bool; 7],
    /// Minutes after midnight (UTC).
    minute: u32,
    pub(super) action: SchedAction,
}

impl SchedEntry {
    /// Does this entry fire at exactly minute-of-week `mow`?
    pub(super) fn fires_at(&self, mow: u32) -> bool {
...
pub(super) fn local_minute_of_week() -> u32 {
    #[cfg(unix)]
    {
...
pub(super) fn parse_schedule(json: &str) -> Result<Vec<SchedEntry>> {
...
pub(super) fn effective_state(entries: &[SchedEntry], now: u32) -> (Option<bool>, Option<u64>) {
...
pub(super) fn next_resume_in(entries: &[SchedEntry], now: u32) -> Option<u32> {
    entries
        .iter()
...
pub(super) fn apply_action(d: &Arc<Daemon>, a: SchedAction) {
    match a {
        SchedAction::Pause | SchedAction::Resume => {
...
pub(super) fn offline_pause_transition(
    going_offline: bool,
    paused: bool,
