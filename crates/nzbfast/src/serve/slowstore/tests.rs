...
fn mkjob(nzo_id: &str, out_dir: &Path) -> Arc<Mutex<crate::serve::job::Job>> {
    let mut j = crate::serve::job_from_json(&json!({
        "nzo_id": nzo_id,
...
fn offline_stands_the_whole_feature_down() {
    ...
    assert!(engage(&d, &ev, &d.out_dir(), &Probe::Slow(3_000)));
...
fn the_named_path_is_the_running_jobs_own_output_directory() {
    ...
    let d = crate::serve::testutil::test_daemon(dir.path());
    assert_eq!(affected_path(&d), d.out_dir(), "no job: the output root");
