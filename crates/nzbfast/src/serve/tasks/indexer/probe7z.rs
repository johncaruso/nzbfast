...
//! neighbours unchanged. Two helpers the rest of indexer.rs still calls
//! (`probe_fetch`, `rar_probe_volumes`) are widened to `pub(super)`;
//! `spawn_probe7z` keeps `pub(in crate::serve)`, which is absolute, and
//! indexer.rs re-exports it so `tasks::spawn_probe7z` still resolves
//! through tasks.rs's glob.
...
#[cfg(feature = "indexer")]
pub(super) async fn probe_fetch(
...
#[cfg(feature = "indexer")]
pub(super) fn rar_probe_volumes(
    files: &[nzbkit::index::ProbeFile],
) -> Vec<nzbkit::index::ProbeFile> {
