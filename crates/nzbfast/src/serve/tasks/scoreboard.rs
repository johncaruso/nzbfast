...
pub(in crate::serve) fn spawn_scoreboard(daemon: &Arc<Daemon>) {
    ...
    tokio::spawn(async move {
        // CLAUDE.md invariant 5: the env gate that fences every other
        // outbound enrichment call fences this one. Checked once - the
        // variable is process-wide and static - so a test daemon can
        // never put a request on the wire.
        if std::env::var_os("NZBFAST_NO_ENRICH").is_some() {
            *d.scoreboard_status.lock_ok() = "disabled by NZBFAST_NO_ENRICH".to_string();
            return;
        }
        loop {
            tokio::time::sleep(std::time::Duration::from_secs(300)).await;
            if !d.scoreboard_enabled.load(Ordering::Relaxed)
            ...
            if let Err(why) = d.scoreboard_reference() {
                *d.scoreboard_status.lock_ok() = why;
                continue;
            }
            let now = unix_now();
            ...
            if d.scoreboard_running.swap(true, Ordering::SeqCst) {
                continue;
            }
            let d2 = d.clone();
            let outcome = tokio::task::spawn_blocking(move || {
                let out = run(&d2);
                d2.scoreboard_running.store(false, Ordering::SeqCst);
                out
            })
            .await;
            match outcome {
