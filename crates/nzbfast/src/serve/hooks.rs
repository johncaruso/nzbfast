...
impl Daemon {
    ...
    pub(in crate::serve) fn notify_event(self: &Arc<Self>, event: &'static str, message: &str) {
    ...
    pub(in crate::serve) fn hooks_offer(&self, event: &Value) {
        let Some(kind) = event["kind"].as_str() else {
            return;
        };
        if !self.notify_targets.lock_ok().iter().any(|t| {
            t.enabled && t.kind == crate::notify::Kind::Webhook && wants_lifecycle(&t.events, kind)
        }) {
            return;
        }
        let tx = self.hooks_tx.lock_ok();
...
pub(in crate::serve) fn wants_lifecycle(events: &[String], kind: &str) -> bool {
    events.iter().any(|e| {
        let e = e.trim();
...
pub(in crate::serve) struct Dispatcher {
...
pub(in crate::serve) fn spawn_dispatcher(d: &Arc<Daemon>) {
    ...
    *d.hooks_tx.lock_ok() = Some(tx);
    ...
    // is counted.
    crate::serve::spawn_aux("hooks", move || disp.run());
}
...
impl Dispatcher {
    ...
    fn lane(&mut self, key: &str) -> Arc<Lane> {
        ...
        crate::serve::spawn_aux("hooks-lane", move || worker.run());
...
fn offer(lane: &Lane, p: Pending) -> Option<Pending> {
...
impl LaneWorker {
    ...
    fn next_due(&self) -> Option<Pending> {
    ...
    fn record(&self, p: &Pending, code: u16, error: String, gave_up: bool) {
        let Some(d) = self.d.upgrade() else { return };
        if error.is_empty() {
