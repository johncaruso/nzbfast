...
mod sidecar_tests {
    ...
    fn a_late_prefetch_ok_never_claims_the_record_it_was_retried_out_of() {
        ...
        let v = serde_json::json!({
            ...
            "out_dir": d.out_dir().join("Prefetched.Release").to_string_lossy(),
    ...
    async fn a_delete_waits_for_the_sidecar_before_removing_its_files() {
        let dir = tmp("drain");
        let d = test_daemon(&dir);
        let out = d.out_dir().join("Prefetching.Release");
    ...
    fn a_recategorize_takes_the_record_out_of_the_prefetch_custody() {
        ...
        let v = serde_json::json!({
            ...
            "out_dir": d.out_dir().join("movies/Some.Release").to_string_lossy(),
            "nzb_path": "/tmp/n.nzb", "state": "Queued",
        });
        let job = Arc::new(Mutex::new(job_from_json(&v).expect("job")));
        let gen0 = job_generation(&job.lock_ok());
        ...
        job.lock_ok().out_dir = d.out_dir().join("tv/Some.Release");
        assert!(
            !sidecar_result_is_ours(&job.lock_ok(), &gen0),
    ...
    async fn a_late_prefetch_tail_never_finalizes_or_parks_the_retry() {
        ...
        // --- the stale direction ---------------------------------------
        let out = d.out_dir().join("Prefetched.Release");
        std::fs::create_dir_all(&out).unwrap();
        ...
        completion_tail(d.clone(), job.clone(), Some((gen0.0, gen0.1)), None).await;
        ...
        completion_tail(d.clone(), job2.clone(), Some((gen2.0, gen2.1)), None).await;
        assert_eq!(
            job2.lock_ok().state,
            JobState::Completed,
    ...
    async fn the_drain_never_removes_while_the_prefetch_still_holds() {
        ...
        let d = test_daemon(&dir);
        let out = d.out_dir().join("Slow.Release");
        std::fs::create_dir_all(&out).unwrap();
        std::fs::write(out.join("part01.rar"), b"bytes").unwrap();
    ...
    async fn the_drain_waits_for_the_detached_completion_tail_too() {
        ...
        let d = test_daemon(&dir);
        let out = d.out_dir().join("Finalizing.Release");
        std::fs::create_dir_all(&out).unwrap();
        std::fs::write(out.join("part01.rar"), b"bytes").unwrap();
    ...
    fn a_recategorize_takes_the_prefetched_progress_with_it() {
        ...
        let gen0 = job_generation(&job.lock_ok());
        let to = d.out_dir().join("tv/Some.Release");
        ...
        let gen2 = job_generation(&job2.lock_ok());
        let to2 = d.out_dir().join("tv/Other.Release");
        {
            let mut g = job2.lock_ok();
            g.out_dir = to2.clone();
            g.retries += 1;
    ...
    fn a_recategorize_stops_the_prefetch_it_re_pointed() {
        ...
        *d.sidecar.lock_ok() = Some(Sidecar {
        ...
        super::api::queue::requeue_category(&d, &job, "Some.Release", "films")
            .expect("recategorize");
        assert!(
