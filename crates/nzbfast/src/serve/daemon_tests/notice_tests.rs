...
fn delete_kept_mutations_move_the_queue_rev() {
    with_daemon("dkrev", |d| {
        ...
        let served = super::sabcompat::queue_json(d, &std::collections::HashMap::new());
        ...
        let r1 = rev();
        assert!(
            !super::api::queue::spend_kept_notice(d, r"C:\Users\GS\Downloads\nobody"),
            "spending a path that is not in the ring changes nothing"
        );
        assert_eq!(rev(), r1, "a no-op must not bump");

        assert!(
            super::api::queue::spend_kept_notice(d, &sent),
